...
app.get('/', (req, res) => {
  ...
  if (alcoholType) {
    for (let item in cocktails) {
      if (cocktails[item].contains.indexOf(alcoholType) > -1) {
        alcoholList.push(cocktails[item]);
      }
    }
  } else {
    alcoholList = cocktails;
  }
  res.render('posts', { alcoholList });
});
